...
userName03,user03-mockAnswer01,ans02`
        ...
        const imageUrls = `url01,url02`
        const painters = `p01,p02`
        // where
        const rs = TemplateGeneratorHelper.parseInput(userMocks, answers, imageUrls,painters);
        

        // then
        expect(rs[0].realAnswer).toBe('ans01')
        expect(rs[0].imageUrl).toBe('url01')
        expect(rs[0].options
                .some(o => 
        ...
        expect(rs[1].imageUrl).toBe('url02')
